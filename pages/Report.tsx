...
const CustomTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    return (
      <div className="bg-black/60 backdrop-blur-xl border border-white/10 p-3 rounded-2xl shadow-2xl animate-fade-in-up">
    ...
    <div className="flex flex-col pb-44">
      ...
      <main className="px-5 flex flex-col gap-8 animate-fade-in-up">
        ...
        <section className="relative w-full aspect-[4/3] rounded-[2.5rem] overflow-hidden shadow-2xl group border border-white/5">
          <img src="https://picsum.photos/seed/mystic-face/800/600" className="absolute inset-0 w-full h-full object-cover opacity-40 group-hover:scale-110 transition-transform duration-1000" alt="Face Map" />
        ...
        <section className="rounded-[2.8rem] p-[1.5px] bg-mystic-gradient shadow-2xl shadow-primary/10">
          <div className="bg-[#1a0b1a] rounded-[2.7rem] p-8 flex flex-col items-center relative overflow-hidden">
            ...
            <div className="flex items-baseline justify-center gap-1 mb-6">
              <span className="text-7xl font-black text-transparent bg-clip-text bg-mystic-gradient tracking-tighter">{data.score}</span>
              ...
              <ResponsiveContainer width="100%" height="100%">
                <RadarChart cx="50%" cy="50%" outerRadius="80%" data={data.radarData}>
                  ...
                  <Radar 
                    ...
                    animationBegin={300}
                    activeDot={{ r: 6, fill: '#fff', stroke: '#ec13ec', strokeWidth: 3 }}
                  />
                </RadarChart>
              </ResponsiveContainer>
            </div>
                ...
                <div key={i} className="relative group">
                  ...
                  <div className="flex flex-col gap-4">
                    <div className="flex items-center">
                      <span className="text-[9px] font-black text-secondary/80 bg-secondary/10 px-2.5 py-1 rounded-lg border border-secondary/10 uppercase tracking-widest">
                        {cite.source}
                      </span>
                    </div>
                    <p className="text-xl font-black text-white leading-snug italic tracking-tight">
                      “{cite.quote}”
                    </p>
            ...
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={data.fortuneTrend} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <Area 
                  ...
                  activeDot={{ r: 8, fill: '#fff', stroke: '#ec13ec', strokeWidth: 4, className: 'animate-pulse' }}
                />
              </AreaChart>
            </ResponsiveContainer>
          </div>
          ...
          <div className="space-y-4">
            {data.advices.map((advice, i) => (
              <div key={i} className="flex gap-5 bg-white/5 p-5 rounded-[2rem] border border-white/5 active:scale-[0.98] transition-all group">
                <div className="shrink-0 size-14 rounded-2xl bg-[#1a0b1a] border border-white/5 flex items-center justify-center shadow-inner group-hover:scale-110 transition-transform">
      ...
      {/* 底部操作栏 */}
      <div className="fixed bottom-0 left-0 right-0 p-6 pb-10 z-50">
